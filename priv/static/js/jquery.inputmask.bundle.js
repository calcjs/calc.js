...
* Version: 3.1.0
*
* Modified by: Vaiko Karusion (vaiko_karusion@co-graph.com)
* Last modified date:  26/08/2014
*
* Added functionality:
*     1) added functionality to remove the exessive trailing radixpoint on blur
*     2) added functionality to replace the "-,", "-." and "." on blur
*
* Modified functionality: Input Mask plugin extensions ('decimal' and 'integer' alias) 
*     1) changed the default values of integer and decimal filter
*     2) enabled the exessive trailing radixpoint removal
*     3) enabled the output cleaning on blur 
*/
    ...
    if ($.fn.inputmask === undefined) {
            ...
            function mask(el) {
                ...
                if ($el.is(":input") && $el.attr("type") != "number") {
                    ...
                    $el.bind("mouseenter.inputmask", function () {
                        ...
                        //added by: Vaiko Karusion
                        //remove the exessive trailing radix point
                        if (opts.trimTrailingRadix) {
                            var tmpBuffer = getBuffer().join('');
                            
                            if (tmpBuffer.length >= 2) {
                                var radixPosition = tmpBuffer.indexOf(opts.radixPoint);
                            
                                if (radixPosition != -1 && radixPosition == (tmpBuffer.length - 1)) {
                                    $input.val(tmpBuffer.substring(0, tmpBuffer.length - 1));
                                }
                            }
                        }
                        //added by: Vaiko Karusion
                        //output cleaning on blur
                        //Notice! Order of operations is important.
                        if (opts.cleanNumberOnBlur) {
                            var tmpBuffer = getBuffer().join('');
                            
                            if (tmpBuffer.length > 2) {
                                //replace "-," with "-"
                                if (tmpBuffer.substring(0, 2) == "-,") {
                                    $input.val(tmpBuffer.replace("-,", "-"));
                                }
                                
                                //replace "-." with "-0."
                                if (tmpBuffer.substring(0, 2) == "-.") {
                                    $input.val(tmpBuffer.replace("-.", "-0."));
                                }
                            }
                            
                            if (tmpBuffer.length > 1) {
                                //replace "." with "0."
                                if (tmpBuffer.substring(0, 1) == ".") {
                                    $input.val(tmpBuffer.replace(".", "0."));
                                }
                            }
                        }
        ...
        $.inputmask = {
            ...
            defaults: {
                ...
                trimTrailingRadix: false, //remove the exessive trailing radixpoint on blur (added by: Vaiko Karusion)
                cleanNumberOnBlur: false, //clean value on blur (added by: Vaiko Karusion)
...
(function (factory) {
    ...
    $.extend($.inputmask.defaults.aliases, {
        'numeric': {
            ...
            digitsOptional: true,
            groupSeparator: ",",//",", // | "." //original value: "" (changed by: Vaiko Karusion)
            radixPoint: ".",
            groupSize: 3,
            autoGroup: true, //original value: false (changed by: Vaiko Karusion)
            allowPlus: false, //original value: true (changed by: Vaiko Karusion)
            allowMinus: true,
            integerDigits: "+", //number of integerDigits
            prefix: "",
            suffix: "",
            rightAlign: true,
            trimTrailingRadix: true, //added by: Vaiko Karusion
            cleanNumberOnBlur: true, //added by: Vaiko Karusion
            ...
            insertMode: true,
            autoUnmask: true, //original value: false  (changed by: Vaiko Karusion)
